...
bool OBJLoader::init (void) 
{
	...
	while(!feof(f)) {
		...
		if (!tokens[0].compare("v")) {
			...
			for (unsigned int i = 1; i <= 3; ++i) {
				tVertices.push_back(atof(tokens[i].c_str()));
			}

		} else if (!tokens[0].compare("vt")) {
			//texture co-ordinate
			for (unsigned int i = 1; i <= 2; ++i) {
				tTextureCoordinates.push_back(atof(tokens[i].c_str()));
			}

		} else if (!tokens[0].compare("vn")) {
			//vertex normal
			for (unsigned int i = 1; i <= 3; ++i) {
				tNormals.push_back(atof(tokens[i].c_str()));
			}
	...
	for (int i = 0; i < tIndices.size() - 1; ++i) {
		...
		for (int j = i + 1; j < tIndices.size(); ++j) {
